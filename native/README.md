...
This is native SwiftUI, SwiftData, UIKit and PencilKit source, not a WebView. Build `6` targets iPhone and iPad on iOS/iPadOS 26 under `com.thinkhale.sideleaf`. The app and XCTest products are validated compile-only; no simulator was booted, so XCTest did not execute and runtime UI, physical iPhone/iPad, touch ink, Apple Pencil and microphone behavior remain unvalidated.
...
- Validate build `6` sign-in restoration, legal-version transitions, Keychain cleanup, explicit guest-page adoption, reconnect retries and two-client conflict recovery against the hosted API.
