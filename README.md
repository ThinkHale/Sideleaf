...
- Native build `8` source rebuilds the iPhone app around the meeting: one Start action, live follow-up questions and next steps derived on device by written rules over what was said, a recap that becomes anchored marks on a page, and opt-in local reminders. It adds a Home Screen and Lock Screen widget with an iOS control, Siri phrases, and an Apple Watch companion carrying start, stop and the current questions. Build `8` has not been compiled: it was written without a Swift toolchain, so none of it has been exercised. See [native setup](native/README.md).
