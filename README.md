...
- Better Auth sign-up/sign-in locally, explicit email/password opt-in for the hosted beta, isolated accounts, optional Google authentication configuration, server-enforced ownership, and versioned Terms/recording-responsibility acceptance. Hosted authentication rate limits use database storage shared across Vercel instances.
...
- Native SwiftUI/SwiftData/PencilKit source for iPhone and iPad, with email/password sign-in, account-scoped offline caching, versioned legal acceptance, revision-based synchronization for typed text, semantic marks and saved on-device transcript text, plus native anchor tests. Build `6` targets iOS/iPadOS 26 or later and includes the audio-engine lifecycle hardening for the reported microphone-start crash. No simulator was booted, XCTest did not execute, and physical-device behavior remains unverified. PencilKit ink remains device-only.
- Native build `8` source rebuilds the iPhone app around the meeting: one Start action, live follow-up questions and next steps derived on device by written rules over what was said, revised as the conversation answers them, a recap that becomes anchored marks on a page, and opt-in local reminders. What the person asks, keeps or dismisses changes what is suggested next, as a visible tally that can be cleared. An optional voice profile, recorded once in settings, lets Sideleaf tell the owner's voice from the room's, so a promise they made and a promise they were made stop landing on the same list. It adds a Home Screen and Lock Screen widget with an iOS control, Siri phrases, and an Apple Watch companion carrying start, stop and the current questions. Build `8` compiles and archives cleanly in Xcode Cloud, but has never run: no test has executed, and its export is blocked until the widget and watch bundle identifiers and the Sideleaf App Group are registered with Apple. See [native setup](native/README.md).
...
The current `npm run check` pass includes the production build and 135 passing Vitest tests across 13 files. Two focused legal-acceptance browser flows also passed; the complete browser end-to-end and built-PWA suites were updated but were not rerun in this pass. Hosted synthetic speech previously passed the real OpenAI connection, server-confirmed transcript storage, microphone pause and session closure. Synthetic test accounts were removed. Detailed evidence and remaining native/payment validation are recorded in [status.md](docs/status.md).
...
Live web transcription and assisted usage accounting are implemented. Account creation now requires acceptance of the current Terms and a recording-law responsibility acknowledgement; later Terms versions gate sync and capture while preserving sign-out, subscription management, account export, and deletion. Native sign-in and revision-based synchronization are implemented for typed text, semantic marks and saved on-device transcript text; PencilKit ink remains local until owned artifact routes and a shared editing contract exist. Real text-AI coaching and grounded summaries remain unfinished; build `8`'s meeting suggestions are deterministic rules over the transcript, not inference. The universal iPhone/iPad build `8`, its widget and its watch companion have never been compiled, and the App Group and background-audio capabilities are not yet provisioned. Stripe purchase activation and native store billing need their provider setup and end-to-end validation. The universal iPhone/iPad build `6` has not been exercised in a booted simulator or on physical iPhone/iPad hardware. The API and legal-page changes in this commit also require hosted deployment verification and launch counsel review.
