...
- **No part of build `8` was compiled or run.** It was written in a Linux session with no Swift toolchain, Xcode, simulator, watch or phone, so the claims below describe source and intent only. Everything here needs a Mac before it can be called working.
...
- `MeetingIntelligenceTests`, `MeetingDatesTests` and `MeetingRecapTests` were added, including the invariant that every mark a recap creates anchors to the exact words that were said. **None of these tests has executed.**
