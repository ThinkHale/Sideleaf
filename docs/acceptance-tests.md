...
### Speaker attribution and the voice profile, build 8

- Cues that describe work now carry an owner and how it was decided. A promise belongs to whoever made it; a request belongs to whoever it was made of. The recap splits "Your next steps" from "What they owe you", and a cue Sideleaf could not attribute says "Assumed yours" and moves with one tap. `MeetingAttributionTests` covers the rule and the labels; the recap tests cover the split. **None of these tests has executed.**
- Who is speaking comes from an optional voice profile recorded in Settings. There is no Apple API for speaker identification on iOS 26, so this uses FluidAudio's Sortformer diarizer, the native app's first third-party dependency, pinned to its minor series.
- The transcriber keeps its single tap. Attribution takes a copy through a bounded lock-free sink that never blocks the render thread, and nothing is attached at all unless a profile exists. **Whether the fan-out disturbs transcription is unverified, and that is the risk worth watching first.**
- Attribution answers only when one voice held at least 70 per cent of the window that produced the text, offset by an assumed 1.5 second transcript lag. Both numbers were reasoned, not measured.
- The voice profile is the only recording Sideleaf stores: about 25 seconds, in the app container with complete file protection, deleted from the same screen. Meeting audio is still never written to disk. The microphone purpose string was rewritten to say both, and the CI assertion that pins that string was updated with it. Other participants are distinguished within a meeting and never enrolled or stored.
- First setup downloads speaker models from their publisher. No audio leaves the device at any point.
- **Nothing in this section has been compiled or run.** The adapter was written against FluidAudio's published API documentation with the library absent, so its signatures are unconfirmed, and no voice has been enrolled, matched or misattributed.
