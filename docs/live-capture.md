...
Build `8` moves native capture out of a per-page sheet and into a meeting: the person picks a kind, optionally writes down what the conversation is for, starts once, and reviews a recap when it ends. A deterministic on-device engine reads the finalized transcript text as it arrives and offers follow-up questions, commitments, decisions, named dates and uncovered plan points. It matches written rules against the words that were said, keeps the source sentence for every suggestion, sends nothing anywhere, and is not a model inference. The app also declares the `audio` background mode so a meeting is not cut short by a locked screen; that behavior is unverified on hardware.
