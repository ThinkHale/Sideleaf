...
Updated September 9, 2026. Sideleaf's authenticated notebook beta is hosted at [sideleaf.vercel.app](https://sideleaf.vercel.app). Browser live transcription passed an earlier hosted Chromium test using synthetic microphone input, the real OpenAI service and actual Supabase persistence. Native build `6` targets iPhone and iPad on iOS/iPadOS 26 or later under the registered `com.thinkhale.sideleaf` identifier. It adds versioned Terms acceptance and hardens the audio-engine tap lifecycle implicated in the reported microphone-start crash. Stripe credentials and a real sandbox purchase are still pending. XCTest execution, simulator runtime, physical-device microphone behavior and the new hosted deployment remain separate verification work.
...
- Native SwiftUI/PencilKit source for iPhone and iPad. Build `6` includes email/password sign-in through the Sideleaf API, a signed Better Auth bearer stored in Keychain, explicit adoption of device-only guest pages, versioned legal gating, and conflict-safe synchronization of typed text and semantic marks. The sync adapter preserves remote blocks and web ink it does not own; PencilKit freehand ink remains device-only.
...
- Native iOS/iPadOS 26 live transcription processes bounded microphone buffers on-device without creating an audio file or uploading audio. Its transcript text may synchronize as personal user content rather than trusted server/provider transcript provenance. Build `6` includes audio-engine configuration-change reconnection and route-derived tap-buffer sizing; physical-device audio behavior remains to be verified.
...
- An unsigned generic iOS Release archive passed. Its app metadata reports bundle ID `com.thinkhale.sideleaf`, version `1.0` (`6`), minimum OS `26.0`, device families `[1, 2]`, microphone and speech usage descriptions, iPhone/iPad AppIcon entries and Boolean `ITSAppUsesNonExemptEncryption = false`.
