...
Updated September 22, 2026. Sideleaf's authenticated notebook beta is hosted at [sideleaf.vercel.app](https://sideleaf.vercel.app). Browser live transcription passed an earlier hosted Chromium test using synthetic microphone input, the real OpenAI service and actual Supabase persistence. Native build `8` targets iPhone and iPad on iOS/iPadOS 26 or later under the registered `com.thinkhale.sideleaf` identifier, and adds a WidgetKit extension and a watchOS companion. It rebuilds the app around the meeting itself: a deterministic on-device engine offers follow-up questions and next steps while the conversation runs, and a recap turns what the person keeps into anchored marks on a page. Build `8` was written in a Linux session with no Swift toolchain, Xcode, simulator or device. Xcode Cloud build 14 has since compiled and archived it cleanly, so the code builds for iOS 26 and watchOS 26 and produces the intended bundle layout; that build then **failed at export because the widget and watch bundle identifiers and the `group.com.thinkhale.sideleaf` App Group are not registered with Apple**, which also blocks the previously exportable iPhone app. **No test has executed and no build has run**: every claim about behavior below describes intent and source. Build `7` remains the last compile-validated build; it added versioned Terms acceptance, hardened the audio-engine tap lifecycle, and removed the Swift 6 main-actor isolation trap from AVFoundation's real-time microphone callback. Stripe credentials and a real sandbox purchase are still pending. XCTest execution, simulator runtime, and physical-device microphone behavior remain separate verification work.
...
- Native build `8` source opens on a meeting rather than a notebook: a plan (kind, goal, points to cover), a live view that puts suggested questions first and the transcript last, and a recap that is reviewed before anything is written. `MeetingCueEngine` derives suggestions from written rules over the transcript text, keeps the sentence each one came from, and is not a model inference. It also revises them: later speech can answer, date, correct, clear or reverse an open suggestion, and the words that did so are kept on it and shown, with a one-tap way to disagree. The person's verdicts feed back into what is offered next, within the meeting and across meetings, as a visible and clearable tally rather than a hidden model. Saving a recap appends plain text to the page's personal block and anchors `follow-up`, `action` and `important` marks to the exact quoted words, so phone-captured follow-ups reach the browser margin through the existing sync contract without a new route or migration. Reminders are opt-in local notifications. None of this has been compiled or run.
- Native build `8` source adds a Home Screen and Lock Screen widget, an iOS control and Siri phrases that start or end a meeting by handing the request to the app, plus a watchOS companion with start, stop, mark and the current questions on the wrist. Both read a small shared snapshot that excludes the transcript. Neither has been compiled or run, and the App Group and background-audio capabilities are not yet provisioned for the team.
- Native build `8` source attributes work to whoever owes it, from an optional voice profile recorded in settings. This is the native app's first third-party dependency (FluidAudio, for speaker diarization, because iOS exposes no such API) and the first recording Sideleaf stores, which is the person's own voice, kept on device and deletable. Meeting audio is still never saved. Nothing about it has been compiled or run.
...
- Compile build `8` on a Mac and run its unit tests. `MeetingIntelligenceTests`, `MeetingDatesTests` and `MeetingRecapTests` cover the cue rules, the date resolver and the invariant that every saved mark anchors to the exact words that were said, but no test has executed.
- Create the `group.com.thinkhale.sideleaf` App Group for the team, confirm the widget shows live meeting state on a device, and confirm the built `Info.plist` declares `UIBackgroundModes` as an array containing `audio` before relying on a meeting surviving a locked screen.
- Exercise the watch companion against a paired watch, including the unreachable-phone fallback, and decide whether starting a meeting from the wrist while the phone is backgrounded is reliable enough to present as a feature.
- Judge the cue rules against real meetings, interviews and classes. The tests fix the contract; only live conversations show whether the suggestions are worth reading, and whether the cooldowns keep the live view quiet enough.
- Verify the `LocalPage` lightweight migration (two new optional attributes) over a store created by an earlier build.
