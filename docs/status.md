...
Updated September 22, 2026. Sideleaf's authenticated notebook beta is hosted at [sideleaf.vercel.app](https://sideleaf.vercel.app). Browser live transcription passed an earlier hosted Chromium test using synthetic microphone input, the real OpenAI service and actual Supabase persistence. Native build `8` targets iPhone and iPad on iOS/iPadOS 26 or later under the registered `com.thinkhale.sideleaf` identifier, and adds a WidgetKit extension and a watchOS companion. It rebuilds the app around the meeting itself: a deterministic on-device engine offers follow-up questions and next steps while the conversation runs, and a recap turns what the person keeps into anchored marks on a page. **Build `8` has not been compiled.** It was written in a Linux session with no Swift toolchain, Xcode, simulator or device, so every claim about it below describes intent and source, not verified behavior. Build `7` remains the last compile-validated build; it added versioned Terms acceptance, hardened the audio-engine tap lifecycle, and removed the Swift 6 main-actor isolation trap from AVFoundation's real-time microphone callback. Stripe credentials and a real sandbox purchase are still pending. XCTest execution, simulator runtime, and physical-device microphone behavior remain separate verification work.
